...
const PixelBlast: React.FC<PixelBlastProps> = ({
    ...
    useEffect(() => {
        ...
        return createAnimationVisibilityController(container, (isVisible) => {
    ...
    useEffect(() => {
        ...
        const computeClear = () => {
            ...
            if (isLight) return { color: new THREE.Color(lightModeBackgroundColor), alpha: 1 };
            if (backgroundColor) return { color: new THREE.Color(backgroundColor), alpha: 1 };
            if (transparent) return { color: undefined, alpha: 0 };
            return { color: new THREE.Color(0x000000), alpha: 1 };
        };

        const applyClearColor = (renderer: THREE.WebGLRenderer) => {
            const { color, alpha } = computeClear();
            if (color) renderer.setClearColor(color, alpha ?? 1);
            else {
                renderer.setClearColor(0x000000, alpha ?? 0);
                renderer.setClearAlpha(alpha ?? 0);
            }
        };
        ...
        if (mustReinit) {
            ...
            if (typeof document !== 'undefined') {
                observer = new MutationObserver(() => applyClearColor(renderer));
    ...
    return (
        <div
            ref={containerRef}
            className={`pixel-blast-container w-full h-full ${className ?? ''}`}
            style={style}
